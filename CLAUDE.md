...
| `getTools()` from the page's OWN context | **Returns every tool the document registered, including origin-scoped ones.** Calling it in the record's top-level context returned `a__file_exhibit`, `b__concede` and the rest alongside the unscoped `read_board`. `exposedTo` filters what a CROSS-ORIGIN frame receives via `fromOrigins`; it does not hide a document's own tools from script running in that document. Verified in Chrome, 31 Aug 2026. This is the empirical face of sec. 4's warning that the partition is not a claim about the built-in agent. |
