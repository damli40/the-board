...
type LogLine = { at: number; text: string; kind: 'goal' | 'refused' | 'notgranted' | 'ok' };
...
export function App() {
  ...
  const runGoal = useCallback(async (goal: string) => {
    setBusy(true);
    setLog((prev) => [...prev, { at: Date.now(), text: `» ${goal}`, kind: 'goal' }]);
    try {
  ...
  return (
    <div className={`min-h-screen bg-neutral-950 text-neutral-200 font-mono text-xs flex flex-col border-4 ${accent.border}`}>
      ...
      <div data-testid="panel-log" className="flex-1 overflow-y-auto p-3 flex flex-col gap-1">
        ...
        {log.map((line, i) => {
          if (line.kind === 'refused' || line.kind === 'notgranted') {
            return (
              <div
                key={i}
                className={`w-full px-2 py-1.5 rounded border-l-4 ${line.kind === 'refused' ? 'border-red-500 bg-red-950/40 text-red-200' : 'border-orange-500 bg-orange-950/30 text-orange-200'}`}
              >
                {line.text}
              </div>
            );
